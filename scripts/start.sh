...
echo "$TIME_NOW > docker run --restart always --name $CONTAINER_NAME -d -p 8080:8080 $IMAGE_NAME" >> $DEPLOY_LOG
docker run --restart always --name $CONTAINER_NAME -d -p 8080:8080 $IMAGE_NAME
